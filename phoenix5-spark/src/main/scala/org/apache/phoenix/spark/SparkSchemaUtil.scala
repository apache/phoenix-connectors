...
object SparkSchemaUtil {
  ...
  def phoenixSchemaToCatalystSchema(columnList: Seq[ColumnInfo], dateAsTimestamp: Boolean = false, escapeColumnFamily: Boolean = false): StructType = {
    val structFields = columnList.map(ci => {
      val structType = phoenixTypeToCatalystType(ci, dateAsTimestamp)
      val columnName = normalizeColumnName(ci.getColumnName, escapeColumnFamily)
      StructField(columnName, structType)
    })
    new StructType(structFields.toArray)
  }

  private def normalizeColumnName(columnName: String, escapeColumnFamily: Boolean ) = {
    ...
    if (unescapedColumnName.indexOf(QueryConstants.NAME_SEPARATOR) < 0) {
      ...
      normalizedColumnName = if (tokens(0) == "0" || escapeColumnFamily) tokens(1) else unescapedColumnName
    }
